...
class JWTBearer(HTTPBearer):
    ...
    async def __call__(self, request: Request):
        ...
        if data["email"] != "admin@gmail.com":
            raise HTTPException(
                status_code=403,
                detail=f"Credenciales son inválidas, usuario ingresado: {data['email']}",
            )
...
class Movie(BaseModel):  # Se define clase
    ...
    id: Optional[int] = (None,)  # Sintaxis None indica que son atributos opcionales
...
def get_movies() -> (
    List[Movie]
):  # -> List[Movie] indica que vamos a retornar una lista de peliculas
...
@app.post("/movies/clase", tags=["Movies-Clase"], response_model=dict, status_code=201)
